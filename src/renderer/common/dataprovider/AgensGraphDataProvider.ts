...
export class AGraphDataProvider extends DynamicGraphDataProvider {
      ...
      this.dbclient.connect(err=>{
        if (err) {
          ...
          this.dbclient.query("match p = (n)-[]-() where id(n) = '3.1415' return p limit 1; ",[], (err)=>{
            if(err) {
              console.log('1111111111111111333333333333333333333333', err)
  ...
  queryTable(q: string, p: any[] = []) {
    ...
    this.dbclient.query(q,p,(err, result)=>{
      ...
      this.fire('begin');
      this.fire('table' , {
        columns: result.fields.map((f)=>{return {name:f.name, type: dt2str(f.dataTypeID)};}),
        rows: result.rows
      });
      this.fire('end');
    })
  ...
  private processRecord(fld: any, graphBuff: GraphData = null) {
    if(fld['edges'] && fld['vertices']) {
      this.processPath(fld)
    }else if(fld['eid']){
      this.processRelationship(fld)
    }else if(fld['vid']){
      this.processNode(fld)
    }else if(fld instanceof Array){
      for(let f of fld) {
        this.processRecord(f)
      }
  ...
  private processRelationship(rel: any) {
  ...
  private processPath(path: any) {
    for(let v of path.vertices) {
      this.processNode(v);
    }

    for(let e of path.edges) {
      this.processRelationship(e);
    }
  }
